...
class PreviewPanel(Panel):
    def update_contents(self):
        self.console.draw_frame(0, 0, self.w, self.h, title='Combat Preview')
        ...
        if player is not None:
            if NameComponent in player:
                self.console.print(x=2, y=2, string=player[NameComponent].name)
            ...
            if CombatComponent in player:
                ...
                self.console.print(x=2, y=3, string=f'HP: {cc.cur_hp:>2d}/{cc.max_hp:>2d}')
                self.console.draw_rect(x=12, y=3, width=18, height=1, ch=0, bg=constants.COLOR_GRAY1)
                self.console.draw_rect(x=12, y=3, width=hp_bar, height=1, ch=0, bg=constants.COLOR_YELLOW)
                self.console.print(x=2, y=4, string=player_result.name, fg=player_fg)

        if target is not None and target is not player:
            if CombatComponent in target:
                if NameComponent in target:
                    self.console.print(x=2, y=6, string=target[NameComponent].name)
                    ...
                    self.console.print(x=2, y=7, string=f'HP: {cc.cur_hp:>2d}/{cc.max_hp:>2d}')
                    self.console.draw_rect(x=12, y=7, width=18, height=1, ch=0, bg=constants.COLOR_GRAY1)
                    self.console.draw_rect(x=12, y=7, width=hp_bar, height=1, ch=0, bg=constants.COLOR_YELLOW)
                    self.console.print(x=2, y=8, string=target_result.name, fg=target_fg)
                ...
                if player is not None and target is not None and target is not player:
                    ...
                    if player_damage is not None:
                        self.console.print(x=12, y=4, string=f'{player_damage} damage', fg=player_fg)

                    if target_damage is not None:
                        self.console.print(x=12, y=8, string=f'{target_damage} damage', fg=target_fg)
                ...
                for i, e in enumerate(entities[:3]):
                    self.console.print(x=2, y=6+i, string=e[NameComponent].name, fg=e[DisplayComponent].fg)
